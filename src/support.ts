...
Cypress.Commands.add('fsReadFile', (path: string, options?: ReadFileOptions) => {
    return cfs.fsReadFile(path, options)
});

Cypress.Commands.add('fsWriteFile', (path: string, content: string, options: fs.WriteFileOptions) => {
...
Cypress.Commands.add('fsCreateDirectory', (path: string, fileOptions?: fst.CreateDirOptions) => {
    return cfs.fsCreateDirectory(path, fileOptions);
});

Cypress.Commands.add('fsDeleteDirectory', (path: string, fileOptions?: fst.DeleteDirOptions) => {
    return cfs.fsDeleteDirectory(path, fileOptions);
});

Cypress.Commands.add('fsCopyFile', ({ path, newPath, mode }: { path: string, newPath: string, mode?: number }) => {
    return cfs.fsCopyFile({ path, newPath, mode });
});

Cypress.Commands.add('fsChmod', ({ path, mode }: { path: string, mode: number }) => {
    return cfs.fsChmod({ path, mode });
});

Cypress.Commands.add('fsAppendFile', ({ path, content }: { path: string, content: string }) => {
    return cfs.fsAppendFile({ path, content });
});

Cypress.Commands.add('fsRename', ({ path, newPath }: { path: string, newPath: string }) => {
