...
export function fsWriteFile(path: string, content: string, options: fs.WriteFileOptions) {
...
export function fsCreateDirectory(path: string, fileOptions?: fst.CreateDirOptions) {
    cy.task('fsCreateDirectory', { path, fileOptions }, { log: false })
}

export function fsDeleteDirectory(path: string, fileOptions?: fst.DeleteDirOptions) {
    cy.task('fsDeleteDirectory', { path, fileOptions }, { log: false })
}

export const fsCopyFile = ({ path, newPath, mode }: { path: string, newPath: string, mode?: number }) => {
...
export const fsAppendFile = ({ path, content }: { path: string, content: string }) => {
    cy.task('fsAppendFile', { path, content }, { log: false })
}

export const fsRename = ({ path, newPath }: { path: string, newPath: string }) => {
