declare namespace Cypress {
    import fs from 'fs';
    type CreateDirOptions = fs.Mode | (fs.MakeDirectoryOptions & {
    ...
    interface Chainable {
        ...
        fsWriteFile(path: string, content: string, options: fs.WriteFileOptions): Chainable<void>;
        fsDeleteFile(path: string): Chainable<void>;
        fsCreateDirectory(path: string, fileOptions?: CreateDirOptions): Chainable<void>;
        fsDeleteDirectory(path: string, fileOptions?: DeleteDirOptions): Chainable<void>;
        fsCopyFile({ path, newPath, mode }: { path: string, newPath: string, mode?: number }): Chainable<void>;
